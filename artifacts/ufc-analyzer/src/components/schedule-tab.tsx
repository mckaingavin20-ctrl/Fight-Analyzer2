  ...
  return (
    <div className="space-y-3">
      ...
      {events.map((ev, i) => {
        ...
        const isMain = ev.name.match(/\bUFC\s+\d+\b/i); // numbered event
        ...
        return (
          <button
            ...
            style={{
              ...
              borderColor: isLive ? 'rgba(34,230,110,0.22)' : isMain ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.07)',
            }}
          >
            {/* Date block */}
            <div
              className="w-12 flex flex-col items-center justify-center rounded-xl py-2 shrink-0"
              style={{
                background: isLive ? 'rgba(34,230,110,0.12)' : 'rgba(255,255,255,0.04)',
              }}
            >
              <span className="text-[9px] font-mono font-black uppercase tracking-widest"
                style={{ color: isLive ? GREEN : 'rgba(255,255,255,0.3)' }}>
                {month}
              </span>
              <span className="text-xl font-black leading-none"
                style={{ color: isLive ? GREEN : 'white' }}>
                {day}
              </span>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <p className={cn(
                  ...
                  isMain ? 'text-white' : 'text-white/80'
                )}>
                  {ev.name}
                </p>
                {isLive && (
              ...
              {(ev.venue || ev.location) && (
                <p className="text-[10px] font-mono text-white/25 mt-1 flex items-center gap-1 truncate">
